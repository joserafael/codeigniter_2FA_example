...
2.  **Environment Setup**
    The project is pre-configured to use a local SQLite database for demonstration purposes.
    Ensure `writable/database.db` and the `writable` folder are writable.
...
-   **Response**: Contains a `secret` (text) and `qr_code_svg` (Base64 encoded SVG image).
    *   **Action**: Scan the QR code with your Authenticator App.
